...
# Source data-fetching (GitHub / Jira) lives in the model layer
# (flowmetrics.source_probe) so it's unit-testable without a web app.
# These thin aliases preserve the call sites + the test-injection
# slots (app.state.probe_*); the route bodies wrap them with caching
# + warehouse-stage enrichment.
from .source_probe import (  # noqa: E402
    GITHUB_LIFECYCLE_EVENTS as _GITHUB_LIFECYCLE_EVENTS,
)
from .source_probe import (  # noqa: E402
    JIRA_LIFECYCLE_EVENTS as _JIRA_LIFECYCLE_EVENTS,
)
from .source_probe import (  # noqa: E402
    bucket_items_by_step as _bucket_items_by_step,
)
from .source_probe import (  # noqa: E402
    dry_run_fetch as _default_dry_run_fetch,
)
from .source_probe import (  # noqa: E402
    probe_source_exists as _default_probe_source,
)
from .source_probe import (  # noqa: E402
    probe_source_vocab as _default_probe_source_vocab,
)


def _default_probe_stages(kind: str, target: dict) -> dict:
    """Legacy stage-discovery probe kept for back-compat with older
    test fixtures. The builder now uses _probe-source-vocab."""
    return {
        "stages": [],
        "hint": (
...
def create_app(
    *,
    ...
    from .web.components.cfd import cfd_daily_metrics_json, flow_balance_spec_json

    templates.env.globals["cfd_daily_metrics"] = cfd_daily_metrics_json
    templates.env.globals["cfd_flow_balance"] = flow_balance_spec_json
