...
> **Diátaxis: Explanation.** What the dashboard actually looks like
> against two real data sources — a GitHub repo (`astral-sh/uv`) and
> an Atlassian Jira project on Apache's public instance
> (`CASSANDRA`). Every chart and control on every page; nothing is
> mocked.

## Home — workflow picker

The landing page lists every configured workflow.
...
**empirical percentiles** (P50, P85, P95) drawn from completed-item
cycle times — not Monte Carlo outputs.
...
![New-workflow wizard](../screenshots/contract-wizard.png)
